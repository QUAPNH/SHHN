...
def tiling(slide_filepath, magnification, patch_size, scale_factor=32, tissue_thresh=0.35, method='adaptive', overview_level=5,
    ...
    # print(f"properties keys: n{slide.properties.keys()}")
    # print(f"properties: n{slide.properties}")
...
def run(args):
    ...
    model_final.load_state_dict(torch.load('C:\\Users\\13262\\Desktop\\HGCN-main\\KimiaNetPyTorchWeights.pth'))
    ...
    all_slide_name = joblib.load('C:\\Users\\13262\\Desktop\\HGCN-main\\data_split\\kirc_split.pkl')
    
    print(f"Slide number: {num_slide}.n")
    print(f"Start tiling ...")
   
    for slide_idx, slide_filepath in enumerate(slide_filepath_list):
        
        # print(str(slide_filepath))
        # cprint(all_slide_name)
       
        if str(slide_filepath).split('\\')[-1].split('.')[0] in all_slide_name[0][0]:
...
def main():
    ...
    parser.add_argument('--slide_dir', type=str, default='C:\\Users\\13262\\Desktop\\TCGA-kirc')
    parser.add_argument('--save_dir', type=str, default='C:\\Users\\13262\\Desktop\\KIRC-EXP\\WSIs')
