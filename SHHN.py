...
from torch_geometric.nn import SAGEConv,LayerNorm,GraphUNet
...
#         self.mask_token = torch.zeros(1, 1, decoder_embed_dim).to(device)
    ...
    def forward(self, x, mask):
        ...
        tmp_x = torch.zeros_like(x).to(x.device)
        Mask_n = 0
        Truth_n = 0
...
class fusion_model_mae_2(nn.Module):
    def __init__(self,in_feats,n_hidden,out_classes,dropout=0.3,train_type_num=3):
        ...
        self.img_gnn_2 = GraphUNet(in_channels=in_feats, hidden_channels=n_hidden, out_channels=out_classes, depth=3, pool_ratios=0.3)        
        self.img_relu_2 = GNN_relu_Block(out_classes)  
        
        self.rna_gnn_2 = GraphUNet(in_channels=in_feats, hidden_channels=n_hidden, out_channels=out_classes, depth=3, pool_ratios=0.3)          
        self.rna_relu_2 = GNN_relu_Block(out_classes)      
        
        self.cli_gnn_2 = GraphUNet(in_channels=in_feats, hidden_channels=n_hidden, out_channels=out_classes, depth=3, pool_ratios=0.3)        
...
#         TransformerConv
        ...
        self.mix = HAttention(query_dim = 512, context_dim = 512, heads = 8, dim_head = 64, dropout = 0.3)
        self.cd = nn.Linear(1536,512)
    ...
    def forward(self,all_thing,train_use_type=None,use_type=None,in_mask=[],mix=False):
        ...
        att_2 = []
        pool_x = torch.empty((0)).to(x_img.device if x_img is not None else x_rna.device if x_rna is not None else x_cli.device)
        if 'img' in data_type:
            x_img = self.img_gnn_2(x_img,edge_index_img) 
            x_img = self.img_relu_2(x_img)   
            batch = torch.zeros(len(x_img),dtype=torch.long).to(x_img.device)
        ...
        if 'rna' in data_type:
            ...
            batch = torch.zeros(len(x_rna),dtype=torch.long).to(x_rna.device)
        ...
        if 'cli' in data_type:
            ...
            batch = torch.zeros(len(x_cli),dtype=torch.long).to(x_cli.device)
        ...
        if len(train_use_type)>1:
            if use_type == train_use_type:
                ...
                k=0
                tmp_x = torch.zeros((len(train_use_type),pool_x.size(1))).to(x_img.device)
            ...
            if mix:
                ...
                allfea = mae_x.view(-1)
                allfea = self.cd(allfea)
                mae_x1 = self.mix(allfea,mae_x[0])
                # mae_x1 = self.activation(mae_x1)
                mae_x2 = self.mix(mae_x1,mae_x[1])
                # mae_x2 = self.activation(mae_x2)
                mae_x3 = self.mix(mae_x2,mae_x[2])
                # mae_x3 = self.activation(mae_x3)
                mae_x = torch.stack((mae_x3,mae_x3,mae_x3),0)
            k=0
        ...
        att_3 = []
        pool_x = torch.empty((0)).to(x_img.device)
        ...
        k=0
        multi_x = torch.empty((0)).to(x_img.device)
